...
Deploy one public Compare endpoint from RocketRide, then replace the supplied HTML bundle's simulated resolve/analyze timers with calls to that endpoint. Service keys stay in RocketRide; the browser receives only the public workflow URL.
